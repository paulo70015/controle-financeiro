...
class TestCarregaPagina:
    def test_titulo_pagina(self, page: Page):
        expect(page).to_have_title(re.compile(r"Controle Financeiro v\d+\.\d+\.\d+"))
...
class TestTrocaAno:
    ...
    def test_aba_ano_atual_ativa(self, page: Page):
        ...
        expect(page.locator(f"#anoTabs a.ativo:has-text('{ANO_TESTE}')")).to_be_visible()

    def test_criar_e_navegar_novo_ano(self, page: Page):
        import random
        from test_browser.helpers import ANO_TESTE

        # Ano futuro com offset aleatorio para evitar colisao entre execucoes
        ...
        # Volta ao ano de teste
        from test_browser.helpers import ANO_TESTE as _ano_teste
        ano_teste_str = str(_ano_teste)
        page.click(f"#anoTabs a:has-text('{ano_teste_str}')")
        wait_for_load(page)
        wait_for_table(page)
